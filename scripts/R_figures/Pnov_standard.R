...
Pn.annot <- "/home/rraborn/scratch/annotations/pnov-full.gff"
Pn.assembly <- "/home/rraborn/scratch/assemblies/pnov_mod.fa"
#Pd.assembly.short <- "/home/rraborn/scratch/assemblies/pdec_scaf_1_10.fa"

# Generate sample sheet
sample_sheet <- data.frame(
  sample_name=stringr::str_glue("Pnov_{seq_len(3)}"),
  file_1=NA, file_2=NA,
  condition=rep("Vegetative", 3)
)

tss.1 <- read.table(file="Pn_TSSset-1.txt", header=TRUE)
tss.2 <- read.table(file="Pn_TSSset-2.txt", header=TRUE)
tss.3 <- read.table(file="Pn_TSSset-3.txt", header=TRUE)

tsr.1 <- read.table(file="/home/rraborn/Para-promoters-analysis/tsr/Pn_TSRset-1.tab", header=FALSE)
tsr.2 <- read.table(file="/home/rraborn/Para-promoters-analysis/tsr/Pn_TSRset-2.tab", header=FALSE)
tsr.3 <- read.table(file="/home/rraborn/Para-promoters-analysis/tsr/Pn_TSRset-3.tab", header=FALSE)
...
Pn.tss <- list(tss.1.gr, tss.2.gr, tss.3.gr)
names(Pn.tss) <- c("Pnov_1", "Pnov_2", "Pnov_3")
...
Pn.tsr <- list(tsr.1.gr, tsr.2.gr, tsr.3.gr)
names(Pn.tsr) <- c("Pnov_1", "Pnov_2", "Pnov_3")

#Creating the TSR explorer object

exp <- tsr_explorer(TSSs=Pn.tss, TSRs=Pn.tsr, 
                    genome_annotation=Pn.annot, genome_assembly=Pn.assembly,
...
plot_threshold_exploration(exp, samples="Pnov_1", point_size=1) + scale_color_viridis_c()
ggsave("Pnov_1_threshold_exploration.png")
plot_threshold_exploration(exp, samples="Pnov_2", point_size=1) + scale_color_viridis_c()
ggsave("Pnov_2_threshold_exploration.png")
plot_threshold_exploration(exp, samples="Pnov_3", point_size=1) + scale_color_viridis_c()
ggsave("Pnov_3_threshold_exploration.png")

#plotting sequence logo for all three replicates
plot_sequence_logo(exp, samples="Pnov_1")
plot_sequence_logo(exp, samples="Pnov_2")
plot_sequence_logo(exp, samples="Pnov_3")
...
ggsave(file="Pnov_correlation_matrix.png") #saving the plot

### plot genomic distribution of TSSs

plot_genomic_distribution(exp, data_type="tss", samples=c("Pnov_1","Pnov_2","Pnov_3")) +
  scale_fill_viridis_d(direction=-1, name="Annotation") 

ggsave(file="Pnov_genomic_distribution.png") #saving the plot

### plot density

plot_density(exp, data_type="tss", samples=c("Pnov_1", "Pnov_2", "Pnov_3"))
...
ggsave(file="Pnov_density.png") #saving the plot

### plot TSS heatmap

plot_heatmap(
  exp, data_type="tss", samples=c("Pnov_1","Pnov_2","Pnov_3"),
  upstream=250, downstream=250,
  use_normalized=TRUE,
  rasterize=TRUE, raster_dpi=150
)

ggsave(file="Pnov_tss_heatmap.png") #saving the plot
...
plot_heatmap(
  exp, data_type="tsr", samples=c("Pnov_1","Pnov_2","Pnov_3"),
  upstream=250, downstream=250,
  use_normalized=TRUE,
  rasterize=TRUE, raster_dpi=150
)

ggave(file="Pnov_tsr_heatmap.png") #saving the plot