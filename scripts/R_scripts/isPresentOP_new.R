# current need: how to order the columns of reduced.ma so that they have expected contents eg by gene
...
#isPresentOP <- function(opDb, tssDb, sppIDs) {
  ...
  new.df <- as.data.frame(new.matrix)
  ...
  #for (i in 1) {
    ...
    this.vec <- df.1
    vec.match <- match(df.1, tsr.vec)
    match.1.out <- this.vec[vec.match]
    #new.df$this.name.1 <- this.vec[match.1.out]
    new.df$this.name.2 <- match.1.out
    ...
    vec.match.2 <- match(df.2, tsr.vec)
    match.2.out <- this.vec[vec.match.2]
    #new.df$this.name.2 <- this.vec[match.2.out]
    new.df$this.name.2 <- match.2.out
#  }
  
  print("this.name.1")
  print(this.name.1)
  #print(head(new.df))
  #print(na.omit(vec.match.2))
  print("length(na.omit(match.2.out))")
  print(length(na.omit(match.2.out)))
  print("this.vec[match.2.out]")
  print(this.vec[match.2.out])
  
  opNames <- rownames(my.matrix)
  
  new.df$OP.names <- opNames
